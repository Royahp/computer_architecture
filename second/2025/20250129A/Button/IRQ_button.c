...
int msb = 0 ;
int XOR =0;
char  arr[] = {0x8F, 0xC7, 0xE3, 0xF1, 0xF8, 0x7C, 0x3E, 0x1F}; 
extern uint16_t bitwiseAffineTransformation (char* A,int b, int c);
//key0
...
////  write code from here with or without debouncing
	...
	timer_value= timer_value &  0xFFFF;
...
//	last_tick1 = tick;
////  write code from here with or without debouncing
	int result= bitwiseAffineTransformation (arr,XOR,0x63);
	LED_Out (result);
	LPC_SC->EXTINT &= (1 << 1);     /* clear pending interrupt         */
}
